...
> 2. **Multi-agent routing — but it doesn't narrow retrieval on its own.** Splitting into the [Step 7 — Multi-agent](.workshop/docs/steps/07-multi-agent.md) group chat does **not** by itself shrink the query: the group chat broadcasts every message to every participant, and the manager only picks who speaks next — it never hands a reformulated, single-topic sub-request. So a grounded specialist's provider still builds its query from the whole shared transcript (the same `top_k` crowding as a single agent, and now the transcript also carries the other specialists' replies). To actually narrow it, pair routing with **option 1** (a per-turn provider on the specialist) or **option 4** (retrieval as a per-destination tool the specialist calls itself).
